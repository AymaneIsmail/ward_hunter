...
import { searchSummonerValidator } from '@/lib/validation/search-summoner'
...
const SearchSummoner: FC<SummonerSearchbarProps> = ({}) => {
  ...
  const processSummonner = async (summonerName: string, region: string) => {
    try {
      ...
      console.log(`/summoner/${validData.region}/${validData.summonerName}`)
      push(`/summoner/${validData.region}/${validData.summonerName}`)
    } catch (error) {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-3">
        <div className="mb-2">
          <select
            ...
            name="region"
          >
            <option defaultChecked value="euw1">
              EUW
            </option>
